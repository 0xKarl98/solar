...
fn validate_summary_manifest_contract_for_servers(
    ...
    validate_npm_runtime_evidence(config, summary)?;
...
fn validate_server_evidence(
    ...
    let override_revision = metadata.source_revision_override.as_deref();
    let override_active = override_revision.is_some();
    if override_active && spec.id != "solar" {
        bail!("summary server evidence contains a Solar override for `{}`", spec.id)
    }
    if let Some(revision) = override_revision
        && (revision.len() != 40 || !revision.bytes().all(|byte| byte.is_ascii_hexdigit()))
    {
        bail!("summary Solar override revision is not a 40-character Git SHA")
    }
    let override_source_url =
        spec.source.as_ref().map_or("workflow://solar", |source| source.url.as_str());
    let source_matches = if override_active {
        metadata.source.as_ref().is_some_and(|source| {
            Some(source.revision.as_str()) == override_revision && source.url == override_source_url
        })
    } else {
    ...
    if metadata.label != spec.label
        || !override_active && metadata.command != command
        ...
        || !source_matches
        || !override_active && metadata.artifact_path.as_ref() != artifact_path
        || !override_active
    ...
    let executable_path = if override_active { &metadata.command } else { &command };
    let executable_sha256 = executable_path
        .is_file()
        .then(|| crate::lifecycle::sha256_path(executable_path).ok())
        .flatten();
    ...
    if metadata.executable_sha256 != executable_sha256
        || !override_active && metadata.artifact_sha256 != artifact_sha256
        || override_active && metadata.artifact_path.is_some()
        || override_active && metadata.artifact_expected_sha256.is_some()
        || override_active && metadata.artifact_sha256.is_some()
    {
    ...
    if metadata.status == ServerStatus::Available {
        ...
        if override_active {
            effective_spec.command = metadata.command.clone();
        ...
        let version = crate::lifecycle::inspect_version(
            if override_active { &metadata.command } else { &command },
            &effective_spec,
            crate::lifecycle::VERSION_PROBE_TIMEOUT,
        )
        .with_context(|| format!("failed to verify server evidence for `{}`", spec.id))?;
        crate::lifecycle::verify_server_version_output(&effective_spec, &version)
            .with_context(|| format!("failed to verify server evidence for `{}`", spec.id))?;
...
fn validate_raw_sample_contract_for_servers(
    ...
    for sample in samples {
        ...
        if let Some(process) = &sample.process {
            ...
            validate_process_exit_status(&sample.status, process, "process")?;
        }
        ...
        for phase in &sample.setup_phases {
            ...
            validate_process_exit_status(&sample.status, &phase.process, "setup process")?;
...
fn validate_process_exit_status(
    status: &RunStatus,
    process: &ProcessMetrics,
    role: &str,
) -> Result<()> {
    let healthy = process.exit_code == Some(0) && !process.forced_kill;
    let forced_timeout_cleanup = matches!(status, RunStatus::Timeout)
        && process.forced_kill
        && process.exit_code.is_none_or(|exit_code| exit_code == 0);
    if healthy || matches!(status, RunStatus::Crash) || forced_timeout_cleanup {
        return Ok(());
    }
    bail!(
        "raw sample has an unhealthy {role} exit inconsistent with `{}` status",
        status_name(status)
    )
}
...
fn validate_npm_runtime_evidence(config: &Config, summary: &SummaryReport) -> Result<()> {
    if !requires_npm_runtime(config, &summary.servers) {
        if summary.npm_runtime.is_some() {
            bail!("summary contains npm runtime evidence without an npm-backed server")
        }
        return Ok(());
    }

    let expected = observe_npm_runtime();
    expected.node_version.as_ref().context("validating Node.js runtime evidence is unavailable")?;
    expected.npm_version.as_ref().context("validating npm version evidence is unavailable")?;
    if summary.npm_runtime.as_ref() != Some(&expected) {
        bail!("summary Node.js/npm runtime evidence does not match the validating environment")
    }
    Ok(())
}
...
fn is_solar_pr_profile(profile: &str) -> bool {
    profile == "pr"
}
...
fn compare_summaries_with_sources(
    ...
    if is_solar_pr_profile(&baseline.profile) || is_solar_pr_profile(&candidate.profile) {
...
fn compatibility_blockers(baseline: &SummaryReport, candidate: &SummaryReport) -> Vec<String> {
    ...
    require_digest_contract(
        ...
        is_solar_pr_profile(&baseline.profile) || is_solar_pr_profile(&candidate.profile),
    );
    for (role, summary) in [("baseline", baseline), ("candidate", candidate)] {
    ...
    compare_npm_runtime_contract(&mut blockers, baseline, candidate);
    compare_contract(
        &mut blockers,
        "operating system",
...
fn pr_solar_provenance_blockers(blockers: &mut Vec<String>, role: &str, summary: &SummaryReport) {
    if !is_solar_pr_profile(&summary.profile) {
        return;
    }
    ...
    let executable_valid = solar.executable_sha256.as_deref().is_some_and(is_sha256_digest);
    let (valid, matching) = if let Some(override_revision) = &solar.source_revision_override {
        let override_valid = is_git_revision(override_revision)
            && solar.source.as_ref().is_some_and(|source| {
                source.revision == *override_revision && !source.url.trim().is_empty()
            })
            && solar.artifact_path.is_none()
            && solar.artifact_expected_sha256.is_none()
            && solar.artifact_sha256.is_none();
        if !override_valid {
            blockers.push(format!("{role} PR summary Solar override provenance is invalid"));
        }
        (executable_valid && override_valid, executable_valid && override_valid)
    } else {
        let digests = [
...
fn compare_npm_runtime_contract(
    blockers: &mut Vec<String>,
    baseline: &SummaryReport,
    candidate: &SummaryReport,
) {
    match (&baseline.npm_runtime, &candidate.npm_runtime) {
        (None, None) => {
            if baseline
                .servers
                .iter()
                .chain(&candidate.servers)
                .any(|server| server.id == "nomic-foundation")
            {
                blockers.push("npm runtime evidence is unavailable".into());
            }
        }
        (Some(baseline), Some(candidate)) => {
            compare_required_contract(
                blockers,
                "Node.js runtime",
                baseline.node_version.as_deref(),
                candidate.node_version.as_deref(),
            );
            compare_required_contract(
                blockers,
                "npm version",
                baseline.npm_version.as_deref(),
                candidate.npm_version.as_deref(),
            );
        }
        _ => blockers.push("npm runtime requirement differs between baseline and candidate".into()),
    }
}
...
fn server_contract(summary: &SummaryReport) -> Vec<ServerContract<'_>> {
    let mut servers = summary
        ...
        .iter()
        .map(|server| {
            let role_specific_solar = is_solar_pr_profile(&summary.profile) && server.id == "solar";
...
"#
    ...
    fn summary_artifact_rejects_unknown_fields() {
        let summary = serde_json::to_value(comparison_summary(BTreeMap::from([(
            "hover".into(),
        ...
        for pointer in [
            ...
            "/workloads/0",
            "/summaries/0",
            "/summaries/0/metrics/hover",
        ] {
            let value = add_unknown_field(summary.clone(), pointer);
            assert!(
                serde_json::from_value::<SummaryReport>(value).is_err(),
    ...
    #[test]
    fn raw_sample_artifact_rejects_unknown_fields() {
        ...
        sample.observations.requests.push(crate::process::RequestMeasurement {
            method: "textDocument/hover".into(),
            elapsed_ms: 1.0,
            process_tree_cpu_ms: Some(0.5),
        });
        sample.observations.events.push(crate::process::TraceEvent {
            elapsed_ms: 1.0,
            direction: crate::process::Direction::Send,
            method: Some("textDocument/hover".into()),
            id: Some(Value::from(1)),
            message: serde_json::json!({"jsonrpc": "2.0"}),
        });
        sample.observations.server_requests.push(crate::process::ServerRequest {
            method: "workspace/configuration".into(),
            handled: true,
            error_code: None,
        });
        let samples = serde_json::to_value(SamplesReport {
            schema_version: RESULT_SCHEMA_VERSION,
            samples: &[sample],
        })
        .unwrap();
        for pointer in [
            ...
            "/samples/0",
            "/samples/0/process",
            "/samples/0/setup_phases/0",
            "/samples/0/correctness/0",
            "/samples/0/observations",
            "/samples/0/observations/requests/0",
            "/samples/0/observations/events/0",
            "/samples/0/observations/server_requests/0",
        ] {
            let value = add_unknown_field(samples.clone(), pointer);
            assert!(
                serde_json::from_value::<OwnedSamplesReport>(value).is_err(),
                "accepted unknown field at `{pointer}`"
            );
        }
    }
    ...
    fn markdown_keeps_failed_groups_visible_without_metrics() {
        ...
        let mut summary = summary_with_groups(vec![group.clone()]);
        summary.workloads.push(WorkloadMetadata {
            id: "correctness".into(),
            fixture: "synthetic".into(),
        ...
        assert!(
            output.contains(
                "| external | synthetic | correctness | textDocument/didChange, textDocument/didSave | 0 | incorrect:1 |"
            ),
            "{output}"
        );
    ...
    #[test]
    fn comparison_requires_same_nomic_node_and_npm_runtime() {
        fn with_npm_runtime(
            summary: &SummaryReport,
            node_version: &str,
            npm_version: &str,
            ...
            value["npm_runtime"] = serde_json::json!({
        ...
        let mut nomic = summary.servers[0].clone();
        nomic.id = "nomic-foundation".into();
        summary.servers.push(nomic);
        let mut nomic_group = summary.summaries[0].clone();
        nomic_group.server = "nomic-foundation".into();
        summary.summaries.push(nomic_group);

        let missing = compare_summaries(
            Path::new("baseline.json"),
            &summary,
            Path::new("candidate.json"),
            &summary,
        ...
        assert!(missing.blockers.iter().any(|blocker| blocker.contains("npm runtime")));

        let baseline = with_npm_runtime(&summary, "v22.18.0", "10.9.3");
        let matching = with_npm_runtime(&summary, "v22.18.0", "10.9.3");
        let changed_node = with_npm_runtime(&summary, "v24.6.0", "10.9.3");
        let changed_npm = with_npm_runtime(&summary, "v22.18.0", "11.5.1");

        let compare = |candidate: &SummaryReport| {
            compare_summaries(
                Path::new("baseline.json"),
                &baseline,
                Path::new("candidate.json"),
        ...
        assert!(compare(&matching).compatible);

        let comparison = compare(&changed_node);
        assert!(!comparison.compatible);
        assert!(comparison.blockers.iter().any(|blocker| blocker.contains("Node.js runtime")));

        let comparison = compare(&changed_npm);
        assert!(!comparison.compatible);
        assert!(comparison.blockers.iter().any(|blocker| blocker.contains("npm version")));
    }
    ...
    #[test]
    fn comparison_rejects_malformed_solar_override_provenance() {
        let baseline = comparison_summary(BTreeMap::from([(
            "hover".into(),
            metric_stats(3, 100.0, 100.0, 120.0),
        )]));
        let mut candidate = baseline.clone();
        candidate.servers[0].source_revision_override = Some("not-a-revision".into());
        ...
        assert!(!comparison.compatible);
        assert!(
            comparison.blockers.iter().any(|blocker| blocker.contains("Solar override provenance"))
        );
    }
    ...
    #[test]
    fn pr_smoke_comparison_accepts_cross_server_results() {
        let mut baseline = comparison_summary(BTreeMap::from([(
            "hover".into(),
            metric_stats(3, 100.0, 100.0, 120.0),
        )]));
        baseline.profile = "pr-smoke".into();
        let mut extra_server = baseline.servers[0].clone();
        extra_server.id = "other".into();
        baseline.servers.push(extra_server);
        let mut extra_group = baseline.summaries[0].clone();
        extra_group.server = "other".into();
        baseline.summaries.push(extra_group);
        let candidate = baseline.clone();
    ...
    #[test]
    fn result_validation_binds_solar_override_source_url() {
        let directory = tempfile::tempdir().unwrap();
        let (config_path, output) = publication_artifacts_for_servers(directory.path(), &["solar"]);
        rewrite_summary(&output, |summary| {
            let solar = &mut summary["servers"][0];
            solar["source_revision_override"] = Value::String("1".repeat(40));
            solar["source"]["url"] = Value::String("https://example.invalid/other.git".into());
            solar["artifact_path"] = Value::Null;
            solar["artifact_expected_sha256"] = Value::Null;
            solar["artifact_sha256"] = Value::Null;
        });
    ...
    #[test]
    fn result_validation_requires_crash_for_unhealthy_process_exit() {
        let directory = tempfile::tempdir().unwrap();
        let (config_path, output) = publication_artifacts_for_servers(directory.path(), &["solar"]);
        let config = Config::load(&config_path).unwrap();
        let published = read_summary(&output.join("summary.json"), "publication").unwrap().summary;
        let samples = serde_json::from_slice::<OwnedSamplesReport>(
            &fs::read(output.join("samples.json")).unwrap(),
        )
        .unwrap()
        .samples;
        let repetitions = BTreeMap::from([("synthetic-warm-hover".into(), 1)]);
        let validate = |samples: &[RunSample]| {
            let summary = summarize(SummaryInput {
                config_path: config_path.clone(),
                config: &config,
                servers: published.servers.clone(),
                fixtures: published.fixtures.clone(),
                samples,
                repeat_override: None,
                workload_repetitions: &repetitions,
                timeout_ms: config.profiles["publish"].timeout_ms,
            ...
            validate_raw_sample_contract_for_servers(
                ...
                &summary,
                samples,
                &BTreeSet::from(["solar".into()]),
            )
        };

        for status in [RunStatus::Unsupported, RunStatus::Incorrect, RunStatus::HarnessError] {
            let mut sample = samples[0].clone();
            sample.status = status;
            sample.error = Some("failed".into());
            sample.process.as_mut().unwrap().exit_code = Some(1);
            let error = validate(&[sample]).unwrap_err().to_string();
            assert!(error.contains("unhealthy process exit"), "{error}");
        }

        let mut setup_failure = samples[0].clone();
        setup_failure.status = RunStatus::Unsupported;
        setup_failure.error = Some("failed".into());
        let mut process = setup_failure.process.clone().unwrap();
        process.exit_code = Some(1);
        setup_failure.setup_phases.push(ProcessPhase {
            name: "cache-population".into(),
            process,
            observations: Observations::default(),
        });
        let error = validate(&[setup_failure]).unwrap_err().to_string();
        assert!(error.contains("unhealthy setup process exit"), "{error}");

        let mut forced_timeout = samples[0].clone();
        forced_timeout.status = RunStatus::Timeout;
        forced_timeout.error = Some("timed out".into());
        let process = forced_timeout.process.as_mut().unwrap();
        process.exit_code = None;
        process.forced_kill = true;
        validate(&[forced_timeout]).unwrap();

        let mut raced_nonzero_exit = samples[0].clone();
        raced_nonzero_exit.status = RunStatus::Timeout;
        raced_nonzero_exit.error = Some("timed out".into());
        let process = raced_nonzero_exit.process.as_mut().unwrap();
        process.exit_code = Some(1);
        process.forced_kill = true;
        let error = validate(&[raced_nonzero_exit]).unwrap_err().to_string();
        assert!(error.contains("unhealthy process exit"), "{error}");
    }
