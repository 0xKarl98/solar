...
pub(crate) struct Config {
    ...
    #[serde(default)]
    pub(crate) servers_lock: Option<PathBuf>,
    #[serde(default)]
    pub(crate) fixtures_lock: Option<PathBuf>,
...
impl Config {
    pub(crate) fn load(path: &Path) -> Result<Self> {
        ...
        if let Some(lock) = &servers_lock {
            ...
            let document = load_yaml::<ServersLock>(lock, "server lock")?;
        ...
        if let Some(lock) = &fixtures_lock {
            ...
            let document = load_yaml::<FixturesLock>(lock, "fixture lock")?;
...
fn load_yaml<T: for<'de> Deserialize<'de>>(path: &Path, kind: &str) -> Result<T> {
    let bytes =
...
fn validate_download_pair(
    ...
    if url.is_some() != digest.is_some() {
        bail!("{kind} URL and SHA-256 must be declared together")
    }
    if url.is_some() && path.is_none() {
        bail!("{kind} URL requires an artifact path")
    }
...
fn validate_step_paths(step: &StepSpec) -> Result<()> {
    match step {
        ...
        StepSpec::Probe { probe, .. } => match probe {
            ...
            ProbeSpec::References { path, .. } | ProbeSpec::DocumentSymbol { path, .. } => {
                validate_relative_path(path, "scenario path")
            }
        },
        StepSpec::Warm { probe, .. } => validate_probe_path(probe),
        StepSpec::Rename { path, probe, .. }
...
fn validate_step_anchors(step: &StepSpec, fixture: &FixtureSpec, workload: &str) -> Result<()> {
    ...
    match step {
        StepSpec::Replace { anchor, probe, .. } | StepSpec::Rename { anchor, probe, .. } => {
            require(anchor)?;
            if let Some(probe) = probe {
...
fn validate_probe_anchors(probe: &ProbeSpec, require: &impl Fn(&str) -> Result<()>) -> Result<()> {
    match probe {
        ...
        | ProbeSpec::References { anchor, .. } => require(anchor),
        ProbeSpec::DocumentSymbol { .. } => Ok(()),
    }
}
